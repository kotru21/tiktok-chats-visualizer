...
- **Runtime**: [Bun](https://bun.sh) — сверхбыстрый JavaScript runtime
- **Frontend**: HTML, CSS, TypeScript/JavaScript (ES6+ модули), Chart.js, Vite
- **Обработка данных**: Web Workers (фоновая обработка без блокировки UI)
- **Тестирование**: bun:test (встроенный тест-раннер)
...
- [Bun](https://bun.sh) (версия 1.0 или выше)
...
В проекте используется встроенный тест-раннер Bun. Запуск тестов:
...
Проект использует модульную клиентскую архитектуру:
...
- **workers/** - Web Workers для фоновой обработки данных
...
- **dataProcessor.worker.ts** - обработка данных чатов в отдельном потоке
- Не блокирует UI при загрузке больших файлов
- Все данные обрабатываются локально в браузере

### Types (Общие типы)

- **types/** - интерфейсы и типы для всего приложения
